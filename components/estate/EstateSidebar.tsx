...
import type { UnitStatus } from "@/types/estate";
...
export function EstateSidebar() {
  const { model, filters, setFilters, shading, setShading, statuses, selectUnit, setFocusedAmenity } = useEstate();
  const sectors = [...new Set(model.plots.map((unit) => unit.s))].sort();
  const matches = filters.search ? model.plots.filter((unit) => unit.id.toLowerCase().includes(filters.search.toLowerCase())).slice(0, 8) : [];
  return <aside className="estate-sidebar">
    <FilterGroup title="Find a plot">
      <div className="plot-search"><input value={filters.search} onChange={(event) => setFilters({ ...filters, search: event.target.value })} placeholder="Search plot" /><span>⌕</span></div>
    ...
    <FilterGroup title="Plot size"><div className="filter-chips">{[300, 500, 1000, 3000].map((size) => <button key={size} aria-pressed={filters.sizes.includes(size)} onClick={() => setFilters({ ...filters, sizes: toggle(filters.sizes, size) })}>{size.toLocaleString()} m²</button>)}</div></FilterGroup>
    ...
    <FilterGroup title="Amenities"><div className="amenity-list">{amenities.map((amenity, index) => <button key={amenity.name} onClick={() => setFocusedAmenity(index)}><i style={{ background: amenity.color }} /><span>{amenity.name}</span><small>LOCATE</small></button>)}</div></FilterGroup>
